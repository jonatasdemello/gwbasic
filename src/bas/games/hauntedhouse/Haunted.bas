...
"
This is the program listing for the Haunted House adventure. 
It should run on any computer which uses Microsoft-style BASIC and which 
has a minimum of 16K of RAM. You may have to make a few minor changes 
for your computer - look out for comments next to certain lines in the 
listing. If you have a VIC 20 or an Oric, there are a few extra lines 
and a line change which you will find at the end of the listing on page 
37. If you have a BBC Model A, use mode 7. This listing will not work as 
it is on Sinclair (Timex) computers. If you have a Spectrum (Timex 
2000), turn to page 38 for changes to make to the program. If you have a 
ZX81 (Timex 1000) there is a special listing for you on pages 39 to 45. 
As this is a long program, you will have to be extremely careful when 
you type it in. The smallest mistake could prevent it running properly 
and will be very difficult to find once you ve typed the whole program 
in. Check each line as you go, especially the ON GOSUB and DATA lines. 
Some of the program lines are so long that they take up two or more 
lines on the printed page. Look out for these and make sure you do not 
press RETURN or ENTER until the end of the program line.
"
...
DIM D$(64) : REM  DESCRIPTIONS FOR EACH LOCATION
DIM R$(64) : REM  ROUTES AT EACH LOCATION NSEW

DIM O$(W) : REM  OBJECT DESCRIPTION
DIM V$(V) : REM  VERBS

DIM L(G) : REM  LOCATION OBJECT IS IN
DIM C(W) : REM  OBJECTS CARRYING
DIM F(W) : REM  FLAGS 0/1

DIM RM : REM  LOCATION PLAYER IS IN
DIM M$ : REM  MESSAGES FOR THE PLAYER
DIM Q$ : REM  USER INPUT
DIM V$ : REM  USER INPUT VERB
DIM W$ : REM  USER INPUT WORD
DIM VB : REM  VERB NUMBER
DIM OB : REM  WORD NUMBER
...
1110 IF OB=35 AND RM=43 THEN M$="THERE IS SOMETHING BEYOND.."
...
1175 REM *** SAY ***
1180 M$="OK '"+W$+"'"
1190 IF C(3)=1 AND OB=34 THEN M$="*MAGIC OCCUR*" : IF RM<>45 THEN RM=RND(63)
